...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('last_name', 255)->nullable()->comment('nullable');
            $table->string('email', 255)->unique();
            $table->timestamp('email_verified_at')->nullable()->comment('nullable');
            $table->string('password', 255);

            $table->string('phone', 20)->nullable()->comment('nullable');
            $table->string('cover_image', 255)->nullable()->comment('nullable');
            $table->string('profile_image', 255)->nullable()->comment('nullable');

            $table->enum('role', ['superadmin', 'admin', 'employee', 'user'])->default('user')->comment('User role: superadmin, admin, employee, user');
